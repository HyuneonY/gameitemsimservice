...
router.delete('/character/:id', authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (character.accountId !== accountId) {
      return res.status(403).json({ message: '다른 사용자의 캐릭터입니다.' });
    }
...
/** 캐릭터 상세 조회 API */
router.get('/character/:id', authMiddleware, async (req, res, next) => {
  const characterId = +req.params.id;
  const accountId = req.user.id;
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const reponse = Object.values(itemCountMap);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: '게임 머니를 벌기 중 오류가 발생했습니다.' });
    }
  }
);
